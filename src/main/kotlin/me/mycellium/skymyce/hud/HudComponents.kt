...
fun hudPanel(padding: Int = 5, decorated: Boolean = true): FlowLayout = hudColumn().apply {
    ...
    surface(Surface.flat(0x88000000.toInt()).let { background ->
        if (!decorated) background else background.and(Surface.outline(0x5555FFFF)).and { graphics, panel ->
            graphics.fill(panel.x(), panel.y(), panel.x() + 2, panel.y() + panel.height(), 0xFF55FFFF.toInt())
        }
    })
}

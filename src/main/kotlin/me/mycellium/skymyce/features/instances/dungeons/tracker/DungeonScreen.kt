...
class DungeonScreen : BaseOwoScreen<FlowLayout>() {
        ...
        return UIContainers.verticalFlow(Sizing.fill(80), Sizing.fill(80)).apply {
            ...
            child(
                UIContainers.horizontalFlow(Sizing.fill(), Sizing.content())
                    .child(UIComponents.label(Component.literal("§b§l${DungeonTracker.currentFloor?.name} Tracker Data")).apply {
    ...
    private fun itemScroll(): UIComponent {
        ...
        if (floor == null) {
            ...
            items.forEachIndexed { index, item ->
                ...
                content.child(
                    UIComponents.label(Component.literal("§9${round((item.count.toDouble() / (floor.totalChestsOpened + floor.totalRerolled).toDouble()) * 10000) / 100}%")),
                    index,
                    3
                )
            }
    ...
    private fun timelinePanel(): UIComponent {
        ...
        items.sortedBy { it.time }.forEach { timelineItem ->
            ...
            content.child(UIComponents.item(item).setTooltipFromStack(true))
        }

        return UIContainers.horizontalScroll(Sizing.fill(), Sizing.content(), content).apply {
            padding(Insets.of(3))
            scrollbar(ScrollContainer.Scrollbar.flat(Color.ofRgb(0xFFFFFF)))
            scrollbarThiccness(2)
            surface(HudTheme.panel(true))

...
2. Join SkyBlock and open `/skymyce pf`. The public relay address is already the default. You can inspect or change it under **Settings → Player Relay**; hover Reconnect for the connection status. Leave the address blank to disable it. Connection chatter stays out of normal chat and the list footer.
3. For automatic joining, open **Available**, choose the floor/classes you can play, and save a positive S+ limit such as `5:30.000`. Both participants configure their PB requirements. Fresh shared stats are checked before SkyBlockPv/SkyBlocker or the Hypixel API fallback.
...
6. Incoming LFG requests show the sender, floor, class, and **Yes/No** actions. Yes starts an invitation and automatic acceptance for that specific request. Explicit Yes works with background availability off, but honors a configured PB limit for the requested floor. A plain positive reply to an outstanding LFG also triggers an invitation. Replies do not get invitation buttons. Invite/join confirmations are compact one-line notices. Both players need the updated mod for the complete exchange.

...
5. Press **Party** to send a regular party invitation for the recipient to accept manually. Press **Invite** to send an LFG request with Yes/No actions. The recipient's mod silently acknowledges receipt. Without that receipt within five seconds, or if the recipient/relay is offline, the ordinary LFG text is sent once through Hypixel `/msg`. Party-size and command-cooldown checks still apply.

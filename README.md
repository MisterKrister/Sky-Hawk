...
- **Join** sends a private request asking that friend to invite you for one of your available classes. A friend running this version can offer an open class and invite you back automatically; your mod then accepts the actual server invitation after checking their PB. Both sides need an S+ limit for the same floor. The host also checks your PB, invite permissions, party capacity, and open classes; only friends' structured Join requests trigger this behavior. Requests expire after a minute, and outstanding offers reserve their class and party slot. The host can leave their own Available classes unchecked to host without auto joining other parties.
- **Settings** edits the message template using `{name}`, `{class}`, and `{floor}`. Availability, the PB limit, secondary classes, and the template are saved in `config/skymyce/dungeon_friends.json`. The existing Hypixel API-key setting is reused.

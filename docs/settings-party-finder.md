...
Lookups check the shared relay cache first. Wealth estimates are cached locally and on Cloudflare for 24 hours from the original fetch, and other players with the same friend reuse that estimate. Adding a friend also uses this shared cache. SkyBlockPv remains the provider; local lookups stay at least ten seconds apart, with backoff on API failures and saved estimates retained. A manual refresh can wait up to SkyBlockPv's five-minute profile cache lifetime before fetching fresh data; it does not clear that mod's cache or relabel an old profile as a fresh response. Shared records already fetched within one minute can be reused. Reopening the menu does not start another wealth refresh.
